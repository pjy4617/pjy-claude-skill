...
3개 스킬이 한 번에 활성화됩니다. 이후 에이전트를 설치합니다:

```
/kicad-setup
```

셋업 시 프로젝트 경로를 질문합니다. 지정한 경로에 3개 에이전트와 CLAUDE.md가 생성됩니다.
...
| 에이전트 설치 | "KiCad 설정해줘" | kicad-setup 스킬 |
...
│       │   └── kicad-setup/
│       ├── agents/ (3개)          ← designer, reviewer, bom-generator
│       ├── claude-md/
│       └── docs/
├── CLAUDE.md
├── README.md
