# STM32F4 Firmware Skill & Agent 사용 가이드 (v1.2.0)

## 1. 개요

이 플러그인은 STM32F4 시리즈 MCU(F401/F407/F411/F429/F446) 펌웨어를 **5개 스킬 + 4개 에이전트**로 개발한다.

```
스킬 5개 (절차서)                    에이전트 4명 (전문가)
──────────────                      ─────────────────
...
stm32f4-build     빌드 실행          stm32f4-qa             QA 최종검수
...
이 플러그인은 **STM32CubeIDE for VS Code** (ST 공식 VS Code 확장)를 기본 빌드 환경으로 한다.

- STM32CubeIDE 설치 시 번들되는 **cmake, ninja, arm-none-eabi-gcc** 를 자동 탐지하여 사용
- STM32CubeProgrammer CLI로 플래싱
- VS Code Extensions에서 "STM32 VS Code Extension" 검색하여 설치
...
[빌드 실행]
  "빌드해줘"                  ← stm32f4-build 스킬
    → .cproject / CubeMX Makefile 감지 → CMakeLists.txt 자동 생성
    → STM32CubeIDE 번들 툴체인 탐지 (cmake + ninja + arm-none-eabi-gcc)
    → cmake --preset debug && cmake --build → Flash/RAM 사이즈 리포트
  "빌드하고 플래싱해줘"       ← stm32f4-build --flash
    → STM32CubeProgrammer 또는 OpenOCD로 플래싱
```
...
```
"빌드해줘"
→ stm32f4-build 스킬이 프로젝트 감지
  - .cproject (STM32CubeIDE) → CMakeLists.txt 자동 생성
  - Makefile (CubeMX) → CMakeLists.txt 자동 생성
  - 기존 CMakeLists.txt → 바로 빌드
→ STM32CubeIDE 번들 툴체인 자동 탐지 (cmake + ninja + arm-none-eabi-gcc)
→ cmake --preset debug && cmake --build
→ Flash/RAM 사이즈 리포트

"릴리스 빌드해줘"
→ stm32f4-build release

"빌드하고 플래싱까지 해줘"
→ stm32f4-build --flash (STM32CubeProgrammer 또는 OpenOCD)
```
...
- 빌드 실행은 `stm32f4-build` 스킬로 가능 — STM32CubeIDE 번들 또는 시스템 PATH의 arm-none-eabi-gcc + CMake/Ninja 필요
