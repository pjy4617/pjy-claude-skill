...
wmx3-module-designer가 출력한 설계 산출물을 확인한다. 설계 산출물은 대화 컨텍스트로 전달되거나, `docs/design_<모듈명>.md` 파일로 제공된다.
...
int IM_API mmApiExecute(WMX3_API_PARAM) {
    PIM_LIB_MPARAM lmParam = (PIM_LIB_MPARAM)mParam;
    PMY_MOTOR_DATA pMyData = (PMY_MOTOR_DATA)lmParam->mParam;
    MM_EXECUTE_PARAM* pParam = (MM_EXECUTE_PARAM*)pData;

    if (pMyData == NULL || pParam == NULL) {
        WMX3_API_RETURN_ERROR(MM_ERROR_INVALID_PARAM, 0);
    }
